...
- **Secondary Entity:** `Leave Application Queue`
...
1. **Complete RESTful CRUD API:**
   - `GET /api/employees` — List & search records with multi-field queries.
   - `GET /api/employees/:id` — Retrieve detailed entity record.
   - `POST /api/employees` — Create new validated entity with auto-timestamps.
   - `PUT /api/employees/:id` — Modify existing record with verification.
   - `DELETE /api/employees/:id` — Remove entity from persistent storage.
   - `GET /api/leaves` — Secondary domain service integration.

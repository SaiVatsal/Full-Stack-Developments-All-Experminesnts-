/**
 * WorkforceHR Enterprise Suite - Backend REST API Server
    ...
    if (!fs.existsSync(VERCEL_TMP_DB)) {
      try {
        if (fs.existsSync(DB_FILE)) {
          ...
          fs.writeFileSync(VERCEL_TMP_DB, JSON.stringify({ employees: [], leaves: [] }), 'utf8');
        }
      } catch (e) {}
    }
...
app.get('/api/stats', (req, res) => {
  const db = readDb();
  const items = db.employees || [];
  const secondary = db.leaves || [];
  res.json({
...
app.get('/api/employees/:id', (req, res) => {
  const db = readDb();
  const item = (db.employees || []).find(i => String(i.id) === String(req.params.id));
  if (!item) {
...
app.delete('/api/employees/:id', (req, res) => {
  ...
  res.json({ message: 'Employee Record deleted successfully', id: req.params.id });
});
...
if (require.main === module) {
  app.listen(PORT, () => {
    console.log('======================================================');
    console.log('🚀 WorkforceHR Enterprise Suite is running!');
    console.log('📂 Category: Human Resources');

/**
 * BistroBite Food Delivery Network - Backend REST API Server
    ...
    if (!fs.existsSync(VERCEL_TMP_DB)) {
      try {
        if (fs.existsSync(DB_FILE)) {
          ...
          fs.writeFileSync(VERCEL_TMP_DB, JSON.stringify({ restaurants: [], dispatches: [] }), 'utf8');
        }
      } catch (e) {}
    }
...
app.get('/api/restaurants/:id', (req, res) => {
  const db = readDb();
  const item = (db.restaurants || []).find(i => String(i.id) === String(req.params.id));
  if (!item) {
...
app.delete('/api/restaurants/:id', (req, res) => {
  ...
  db.restaurants = filtered;
  writeDb(db);

  res.json({ message: 'Restaurant & Menu deleted successfully', id: req.params.id });
});
...
app.post('/api/dispatches', (req, res) => {
  ...
  db.dispatches.unshift(newEntry);
  writeDb(db);
...
if (require.main === module) {
  app.listen(PORT, () => {
    console.log('======================================================');
    console.log('🚀 BistroBite Food Delivery Network is running!');
    console.log('📂 Category: Food & Beverage');

/**
 * TradeVision Virtual Stock Trading Sim - Backend REST API Server
    ...
    if (!fs.existsSync(VERCEL_TMP_DB)) {
      try {
        if (fs.existsSync(DB_FILE)) {
          ...
          fs.writeFileSync(VERCEL_TMP_DB, JSON.stringify({ stocks: [], activityRecords: [] }), 'utf8');
        }
      } catch (e) {}
    }
...
app.get('/api/health', (req, res) => {
  res.json({
    status: 'online',
    project: 'TradeVision Virtual Stock Trading Sim',
...
app.get('/api/stocks/:id', (req, res) => {
  const db = readDb();
  const item = (db.stocks || []).find(i => String(i.id) === String(req.params.id));
  if (!item) {
    return res.status(404).json({ error: 'Equities Stock Asset not found' });
  }
  res.json(item);
});
...
app.delete('/api/stocks/:id', (req, res) => {
  ...
  res.json({ message: 'Equities Stock Asset deleted successfully', id: req.params.id });
});
...
if (require.main === module) {
  app.listen(PORT, () => {
    console.log('======================================================');
    console.log('🚀 TradeVision Virtual Stock Trading Sim is running!');
    console.log('📂 Category: FinTech & Stock Trading');

...
- **Secondary Entity:** `Portfolio Order History`
...
1. **Complete RESTful CRUD API:**
   - `GET /api/stocks` — List & search records with multi-field queries.
   - `GET /api/stocks/:id` — Retrieve detailed entity record.
   - `POST /api/stocks` — Create new validated entity with auto-timestamps.
   - `PUT /api/stocks/:id` — Modify existing record with verification.
   - `DELETE /api/stocks/:id` — Remove entity from persistent storage.

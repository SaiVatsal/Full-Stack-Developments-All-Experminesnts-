/**
 * EHR-Nexus Electronic Health Records - Backend REST API Server
    ...
    if (!fs.existsSync(VERCEL_TMP_DB)) {
      try {
        if (fs.existsSync(DB_FILE)) {
          ...
          fs.writeFileSync(VERCEL_TMP_DB, JSON.stringify({ records: [], prescriptions: [] }), 'utf8');
        }
      } catch (e) {}
    }
...
app.get('/api/stats', (req, res) => {
  ...
  const secondary = db.prescriptions || [];
  res.json({
...
app.get('/api/records/:id', (req, res) => {
  const db = readDb();
  const item = (db.records || []).find(i => String(i.id) === String(req.params.id));
  if (!item) {
    return res.status(404).json({ error: 'Patient Clinical Chart not found' });
  }
  res.json(item);
});
...
app.delete('/api/records/:id', (req, res) => {
  ...
  res.json({ message: 'Patient Clinical Chart deleted successfully', id: req.params.id });
});
...
app.post('/api/prescriptions', (req, res) => {
  ...
  db.prescriptions.unshift(newEntry);
  writeDb(db);
...
if (require.main === module) {
  app.listen(PORT, () => {
    console.log('======================================================');
    console.log('🚀 EHR-Nexus Electronic Health Records is running!');
    console.log('📂 Category: Healthcare & EHR');

/**
 * KanbanFlow Agile Task Manager - Backend REST API Server
 * Domain: Productivity
    ...
    if (!fs.existsSync(VERCEL_TMP_DB)) {
      try {
        if (fs.existsSync(DB_FILE)) {
          ...
          fs.writeFileSync(VERCEL_TMP_DB, JSON.stringify({ tasks: [], activityRecords: [] }), 'utf8');
        }
      } catch (e) {}
    }
...
app.get('/api/health', (req, res) => {
  res.json({
    status: 'online',
    project: 'KanbanFlow Agile Task Manager',
...
app.get('/api/tasks/:id', (req, res) => {
  const db = readDb();
  const item = (db.tasks || []).find(i => String(i.id) === String(req.params.id));
  if (!item) {
    return res.status(404).json({ error: 'Agile Task Card not found' });
  }
  res.json(item);
});
...
app.delete('/api/tasks/:id', (req, res) => {
  ...
  res.json({ message: 'Agile Task Card deleted successfully', id: req.params.id });
});
...
if (require.main === module) {
  app.listen(PORT, () => {
    console.log('======================================================');
    console.log('🚀 KanbanFlow Agile Task Manager is running!');

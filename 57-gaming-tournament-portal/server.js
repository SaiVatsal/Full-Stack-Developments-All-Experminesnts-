/**
 * EsportsGrid Tournament Portal - Backend REST API Server
    ...
    if (!fs.existsSync(VERCEL_TMP_DB)) {
      try {
        if (fs.existsSync(DB_FILE)) {
          ...
          fs.writeFileSync(VERCEL_TMP_DB, JSON.stringify({ tournaments: [], activityRecords: [] }), 'utf8');
        }
      } catch (e) {}
    }
...
function readDb() {
  try {
    ...
    return { tournaments: [], activityRecords: [] };
  }
}
...
app.get('/api/health', (req, res) => {
  res.json({
    status: 'online',
    project: 'EsportsGrid Tournament Portal',
...
app.get('/api/tournaments/:id', (req, res) => {
  const db = readDb();
  const item = (db.tournaments || []).find(i => String(i.id) === String(req.params.id));
  if (!item) {
...
app.delete('/api/tournaments/:id', (req, res) => {
  ...
  db.tournaments = filtered;
  writeDb(db);

  res.json({ message: 'Tournament Bracket deleted successfully', id: req.params.id });
});
...
if (require.main === module) {
  app.listen(PORT, () => {
    console.log('======================================================');
    console.log('🚀 EsportsGrid Tournament Portal is running!');
    console.log('📂 Category: Esports & Gaming');

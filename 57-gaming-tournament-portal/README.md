...
- **Secondary Entity:** `Match Bracket Results`
...
1. **Complete RESTful CRUD API:**
   - `GET /api/tournaments` — List & search records with multi-field queries.
   - `GET /api/tournaments/:id` — Retrieve detailed entity record.
   - `POST /api/tournaments` — Create new validated entity with auto-timestamps.
   - `PUT /api/tournaments/:id` — Modify existing record with verification.
   - `DELETE /api/tournaments/:id` — Remove entity from persistent storage.

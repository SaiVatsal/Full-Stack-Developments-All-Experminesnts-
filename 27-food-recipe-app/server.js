/**
 * GourmetCraft Recipe & Cooking App - Backend REST API Server
    ...
    if (!fs.existsSync(VERCEL_TMP_DB)) {
      try {
        if (fs.existsSync(DB_FILE)) {
          ...
          fs.writeFileSync(VERCEL_TMP_DB, JSON.stringify({ recipes: [], activityRecords: [] }), 'utf8');
        }
      } catch (e) {}
    }
...
function readDb() {
  try {
    ...
    return { recipes: [], activityRecords: [] };
  }
}
...
app.get('/api/health', (req, res) => {
  res.json({
    status: 'online',
    project: 'GourmetCraft Recipe & Cooking App',
...
app.get('/api/recipes/:id', (req, res) => {
  const db = readDb();
  const item = (db.recipes || []).find(i => String(i.id) === String(req.params.id));
  if (!item) {
    return res.status(404).json({ error: 'Culinary Recipe not found' });
  }
  res.json(item);
});
...
app.delete('/api/recipes/:id', (req, res) => {
  ...
  res.json({ message: 'Culinary Recipe deleted successfully', id: req.params.id });
});
...
if (require.main === module) {
  app.listen(PORT, () => {
    console.log('======================================================');
    console.log('🚀 GourmetCraft Recipe & Cooking App is running!');
    console.log('📂 Category: Food & Culinary');

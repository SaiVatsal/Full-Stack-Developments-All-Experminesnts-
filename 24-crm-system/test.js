...
const server = app.listen(0, async () => {
  ...
  try {
    ...
    // 2. GET all
    const all = await request('/api/deals');
    console.log(`✔ GET /api/deals: PASSED (${all.data.length} records)`);

    // 3. POST new item
    const postRes = await request('/api/deals', {
      method: 'POST',
    ...
    // 4. PUT update item
    const putRes = await request(`/api/deals/${createdId}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
    ...
    // 5. DELETE item
    const delRes = await request(`/api/deals/${createdId}`, {
      method: 'DELETE'
    });
    console.log('✔ DELETE Item:', delRes.status === 200 ? 'PASSED' : 'FAILED');

    console.log(`🎉 All tests passed successfully for ${app.get ? 'PipelinePro Enterprise CRM' : 'Project'}! Done By SaiVatsal 2500040224`);
    server.close();
    process.exit(0);
  } catch (err) {

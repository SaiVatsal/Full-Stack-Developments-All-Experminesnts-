...
- **Secondary Entity:** `Customer Activity Timeline`
...
1. **Complete RESTful CRUD API:**
   - `GET /api/deals` — List & search records with multi-field queries.
   - `GET /api/deals/:id` — Retrieve detailed entity record.
   - `POST /api/deals` — Create new validated entity with auto-timestamps.
   - `PUT /api/deals/:id` — Modify existing record with verification.
   - `DELETE /api/deals/:id` — Remove entity from persistent storage.

...
const server = app.listen(0, async () => {
  ...
  try {
    ...
    // 3. GET all
    const all = await request('/api/files');
    console.log(`✔ GET /api/files: PASSED (${all.data.length} records)`);

    // 4. POST new item
    const postRes = await request('/api/files', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: {
        fileName: 'Test File Name & Extension',
        fileType: 'Test MIME Classification',
        fileSize: 'Test File Size (MB)',
        owner: 'Test File Owner',
        downloads: 99,
        permission: 'Test Access Policy',
        uploadDate: 'Test Upload Date',
        status: 'Test Link Status'
      }
    });
    ...
    // 5. PUT update item
    const putRes = await request(`/api/files/${createdId}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
    ...
    // 6. DELETE item
    const delRes = await request(`/api/files/${createdId}`, {
      method: 'DELETE'
    });
    console.log('✔ DELETE Item:', delRes.status === 200 ? 'PASSED' : 'FAILED');

    console.log(`🎉 All tests passed successfully for ${app.get ? 'CloudVault Secure File Sharing Platform' : 'Project'}! Done By SaiVatsal 2500040224`);
    server.close();
    process.exit(0);
  } catch (err) {

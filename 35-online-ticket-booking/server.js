/**
 * BoxOffice Cine & Event Ticketing - Backend REST API Server
    ...
    if (!fs.existsSync(VERCEL_TMP_DB)) {
      try {
        if (fs.existsSync(DB_FILE)) {
          ...
          fs.writeFileSync(VERCEL_TMP_DB, JSON.stringify({ shows: [], activityRecords: [] }), 'utf8');
        }
      } catch (e) {}
    }
...
app.get('/api/health', (req, res) => {
  res.json({
    status: 'online',
    project: 'BoxOffice Cine & Event Ticketing',
...
app.get('/api/shows/:id', (req, res) => {
  const db = readDb();
  const item = (db.shows || []).find(i => String(i.id) === String(req.params.id));
  if (!item) {
    return res.status(404).json({ error: 'Concert & Show Listing not found' });
  }
  res.json(item);
});
...
app.delete('/api/shows/:id', (req, res) => {
  ...
  res.json({ message: 'Concert & Show Listing deleted successfully', id: req.params.id });
});
...
if (require.main === module) {
  app.listen(PORT, () => {
    console.log('======================================================');
    console.log('🚀 BoxOffice Cine & Event Ticketing is running!');

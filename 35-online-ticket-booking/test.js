...
const server = app.listen(0, async () => {
  ...
  try {
    ...
    // 2. GET all
    const all = await request('/api/shows');
    console.log(`✔ GET /api/shows: PASSED (${all.data.length} records)`);

    // 3. POST new item
    const postRes = await request('/api/shows', {
      method: 'POST',
    ...
    // 4. PUT update item
    const putRes = await request(`/api/shows/${createdId}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
    ...
    // 5. DELETE item
    const delRes = await request(`/api/shows/${createdId}`, {
      method: 'DELETE'
    });
    console.log('✔ DELETE Item:', delRes.status === 200 ? 'PASSED' : 'FAILED');

    console.log(`🎉 All tests passed successfully for ${app.get ? 'BoxOffice Cine & Event Ticketing' : 'Project'}! Done By SaiVatsal 2500040224`);
    server.close();
    process.exit(0);
  } catch (err) {

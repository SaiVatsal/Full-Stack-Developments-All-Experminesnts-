/**
 * FreshPantry Supermarket & Grocer - Backend REST API Server
    ...
    if (!fs.existsSync(VERCEL_TMP_DB)) {
      try {
        if (fs.existsSync(DB_FILE)) {
          ...
          fs.writeFileSync(VERCEL_TMP_DB, JSON.stringify({ groceries: [], activityRecords: [] }), 'utf8');
        }
      } catch (e) {}
    }
...
function readDb() {
  try {
    ...
    return { groceries: [], activityRecords: [] };
  }
}
...
app.get('/api/health', (req, res) => {
  res.json({
    status: 'online',
    project: 'FreshPantry Supermarket & Grocer',
...
app.get('/api/groceries/:id', (req, res) => {
  const db = readDb();
  const item = (db.groceries || []).find(i => String(i.id) === String(req.params.id));
  if (!item) {
    return res.status(404).json({ error: 'Pantry Grocery Item not found' });
  }
  res.json(item);
});
...
app.delete('/api/groceries/:id', (req, res) => {
  ...
  db.groceries = filtered;
  writeDb(db);

  res.json({ message: 'Pantry Grocery Item deleted successfully', id: req.params.id });
});
...
if (require.main === module) {
  app.listen(PORT, () => {
    console.log('======================================================');
    console.log('🚀 FreshPantry Supermarket & Grocer is running!');
    console.log('📂 Category: Retail & Grocery');

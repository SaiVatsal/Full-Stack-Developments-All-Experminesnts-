/**
 * Vanguard EventHub & Conference Suite - Backend REST API Server
    ...
    if (!fs.existsSync(VERCEL_TMP_DB)) {
      try {
        if (fs.existsSync(DB_FILE)) {
          ...
          fs.writeFileSync(VERCEL_TMP_DB, JSON.stringify({ events: [], registrations: [] }), 'utf8');
        }
      } catch (e) {}
    }
...
app.get('/api/stats', (req, res) => {
  ...
  const secondary = db.registrations || [];
  res.json({
...
app.get('/api/events/:id', (req, res) => {
  const db = readDb();
  const item = (db.events || []).find(i => String(i.id) === String(req.params.id));
  if (!item) {
    return res.status(404).json({ error: 'Conferences & Event not found' });
  }
  res.json(item);
});
...
app.delete('/api/events/:id', (req, res) => {
  ...
  res.json({ message: 'Conferences & Event deleted successfully', id: req.params.id });
});
...
if (require.main === module) {
  app.listen(PORT, () => {
    console.log('======================================================');
    console.log('🚀 Vanguard EventHub & Conference Suite is running!');
    console.log('📂 Category: Events & Hospitality');

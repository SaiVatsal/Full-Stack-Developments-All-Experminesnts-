...
const server = app.listen(0, async () => {
  ...
  try {
    ...
    // 3. GET all
    const all = await request('/api/doctors');
    console.log(`✔ GET /api/doctors: PASSED (${all.data.length} records)`);

    // 4. POST new item
    const postRes = await request('/api/doctors', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: {
        name: 'Test Doctor Full Name & Credentials',
        specialty: 'Test Medical Specialty',
        clinic: 'Test Clinic / Hospital Center',
        rating: 99,
        fee: 99,
        availability: 'Test Consultation Schedule',
        status: 'Test Current Availability'
      }
    });
    ...
    // 5. PUT update item
    const putRes = await request(`/api/doctors/${createdId}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
    ...
    // 6. DELETE item
    const delRes = await request(`/api/doctors/${createdId}`, {
      method: 'DELETE'
    });
    console.log('✔ DELETE Item:', delRes.status === 200 ? 'PASSED' : 'FAILED');

    console.log(`🎉 All tests passed successfully for ${app.get ? 'MediCare Specialist Appointment Portal' : 'Project'}! Done By SaiVatsal 2500040224`);
    server.close();
    process.exit(0);
  } catch (err) {

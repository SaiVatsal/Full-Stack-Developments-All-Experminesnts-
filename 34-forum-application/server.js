/**
 * DevAgora Discussion Forum - Backend REST API Server
 * Domain: Community & Discussions
    ...
    if (!fs.existsSync(VERCEL_TMP_DB)) {
      try {
        if (fs.existsSync(DB_FILE)) {
          ...
          fs.writeFileSync(VERCEL_TMP_DB, JSON.stringify({ threads: [], activityRecords: [] }), 'utf8');
        }
      } catch (e) {}
    }
...
function readDb() {
  try {
    ...
    return { threads: [], activityRecords: [] };
  }
}
...
app.get('/api/health', (req, res) => {
  res.json({
    status: 'online',
    project: 'DevAgora Discussion Forum',
...
app.get('/api/threads/:id', (req, res) => {
  const db = readDb();
  const item = (db.threads || []).find(i => String(i.id) === String(req.params.id));
  if (!item) {
...
app.delete('/api/threads/:id', (req, res) => {
  ...
  res.json({ message: 'Forum Thread deleted successfully', id: req.params.id });
});
...
if (require.main === module) {
  app.listen(PORT, () => {
    console.log('======================================================');
    console.log('🚀 DevAgora Discussion Forum is running!');
    console.log('📂 Category: Community & Discussions');

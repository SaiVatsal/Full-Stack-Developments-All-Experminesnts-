 ...
 * Project: CivicTrust Secure E-Voting Portal
...
const server = app.listen(0, async () => {
  ...
  try {
    ...
    // 3. GET all
    const all = await request('/api/candidates');
    console.log(`✔ GET /api/candidates: PASSED (${all.data.length} records)`);

    // 4. POST new item
    const postRes = await request('/api/candidates', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: {
        candidateId: 'Test Candidate ID',
        name: 'Test Candidate Full Name',
        party: 'Test Affiliation / Party',
        election: 'Test Election Measure Title',
        votes: 99,
        percentage: 'Test Vote Share (%)',
        status: 'Test Race Status'
      }
    });
    ...
    // 5. PUT update item
    const putRes = await request(`/api/candidates/${createdId}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
    ...
    // 6. DELETE item
    const delRes = await request(`/api/candidates/${createdId}`, {
      method: 'DELETE'
    });
    console.log('✔ DELETE Item:', delRes.status === 200 ? 'PASSED' : 'FAILED');

    console.log(`🎉 All tests passed successfully for ${app.get ? 'CivicTrust Secure E-Voting Portal' : 'Project'}! Done By SaiVatsal 2500040224`);
    server.close();
    process.exit(0);
  } catch (err) {

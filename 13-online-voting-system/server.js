/**
 * CivicTrust Secure E-Voting Portal - Backend REST API Server
    ...
    if (!fs.existsSync(VERCEL_TMP_DB)) {
      try {
        if (fs.existsSync(DB_FILE)) {
          ...
          fs.writeFileSync(VERCEL_TMP_DB, JSON.stringify({ candidates: [], auditLogs: [] }), 'utf8');
        }
      } catch (e) {}
    }
...
app.get('/api/stats', (req, res) => {
  const db = readDb();
  const items = db.candidates || [];
  const secondary = db.auditLogs || [];
  res.json({
...
app.get('/api/candidates/:id', (req, res) => {
  const db = readDb();
  const item = (db.candidates || []).find(i => String(i.id) === String(req.params.id));
  if (!item) {
    return res.status(404).json({ error: 'Candidate / Ballot Item not found' });
  }
  res.json(item);
});
...
app.delete('/api/candidates/:id', (req, res) => {
  ...
  res.json({ message: 'Candidate / Ballot Item deleted successfully', id: req.params.id });
});
...
if (require.main === module) {
  app.listen(PORT, () => {
    console.log('======================================================');
    console.log('🚀 CivicTrust Secure E-Voting Portal is running!');
    console.log('📂 Category: Governance & E-Voting');

/**
 * GrandStay Luxury Hotel Booking - Backend REST API Server
 * Domain: Hospitality
    ...
    if (!fs.existsSync(VERCEL_TMP_DB)) {
      try {
        if (fs.existsSync(DB_FILE)) {
          ...
          fs.writeFileSync(VERCEL_TMP_DB, JSON.stringify({ suites: [], activityRecords: [] }), 'utf8');
        }
      } catch (e) {}
    }
...
app.get('/api/health', (req, res) => {
  res.json({
    status: 'online',
    project: 'GrandStay Luxury Hotel Booking',
...
app.get('/api/suites/:id', (req, res) => {
  const db = readDb();
  const item = (db.suites || []).find(i => String(i.id) === String(req.params.id));
  if (!item) {
    return res.status(404).json({ error: 'Hotel Suite Option not found' });
  }
  res.json(item);
});
...
app.delete('/api/suites/:id', (req, res) => {
  ...
  res.json({ message: 'Hotel Suite Option deleted successfully', id: req.params.id });
});
...
if (require.main === module) {
  app.listen(PORT, () => {
    console.log('======================================================');
    console.log('🚀 GrandStay Luxury Hotel Booking is running!');

...
- **Secondary Entity:** `Guest Check-In Schedule`
...
1. **Complete RESTful CRUD API:**
   - `GET /api/suites` — List & search records with multi-field queries.
   - `GET /api/suites/:id` — Retrieve detailed entity record.
   - `POST /api/suites` — Create new validated entity with auto-timestamps.
   - `PUT /api/suites/:id` — Modify existing record with verification.
   - `DELETE /api/suites/:id` — Remove entity from persistent storage.

/**
 * EstatePrime Property Management - Backend REST API Server
 * Domain: Real Estate
    ...
    if (!fs.existsSync(VERCEL_TMP_DB)) {
      try {
        if (fs.existsSync(DB_FILE)) {
          ...
          fs.writeFileSync(VERCEL_TMP_DB, JSON.stringify({ units: [], activityRecords: [] }), 'utf8');
        }
      } catch (e) {}
    }
...
app.get('/api/health', (req, res) => {
  res.json({
    status: 'online',
    project: 'EstatePrime Property Management',
...
app.get('/api/units/:id', (req, res) => {
  const db = readDb();
  const item = (db.units || []).find(i => String(i.id) === String(req.params.id));
  if (!item) {
    return res.status(404).json({ error: 'Property Unit Listing not found' });
  }
  res.json(item);
});
...
app.delete('/api/units/:id', (req, res) => {
  ...
  res.json({ message: 'Property Unit Listing deleted successfully', id: req.params.id });
});
...
if (require.main === module) {
  app.listen(PORT, () => {
    console.log('======================================================');
    console.log('🚀 EstatePrime Property Management is running!');

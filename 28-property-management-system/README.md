...
- **Secondary Entity:** `Tenant Lease Contracts`
...
1. **Complete RESTful CRUD API:**
   - `GET /api/units` — List & search records with multi-field queries.
   - `GET /api/units/:id` — Retrieve detailed entity record.
   - `POST /api/units` — Create new validated entity with auto-timestamps.
   - `PUT /api/units/:id` — Modify existing record with verification.
   - `DELETE /api/units/:id` — Remove entity from persistent storage.

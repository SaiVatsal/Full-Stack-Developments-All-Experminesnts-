/**
 * EduVanguard LMS Platform - Backend REST API Server
    ...
    if (!fs.existsSync(VERCEL_TMP_DB)) {
      try {
        if (fs.existsSync(DB_FILE)) {
          ...
          fs.writeFileSync(VERCEL_TMP_DB, JSON.stringify({ courses: [], submissions: [] }), 'utf8');
        }
      } catch (e) {}
    }
...
app.get('/api/stats', (req, res) => {
  ...
  const secondary = db.submissions || [];
  res.json({
...
app.get('/api/courses/:id', (req, res) => {
  const db = readDb();
  const item = (db.courses || []).find(i => String(i.id) === String(req.params.id));
  if (!item) {
...
app.post('/api/submissions', (req, res) => {
  ...
  db.submissions.unshift(newEntry);
  writeDb(db);
...
if (require.main === module) {
  app.listen(PORT, () => {
    console.log('======================================================');
    console.log('🚀 EduVanguard LMS Platform is running!');
    console.log('📂 Category: EdTech & Training');

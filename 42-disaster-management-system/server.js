/**
 * ResQNet Disaster Coordination Center - Backend REST API Server
    ...
    if (!fs.existsSync(VERCEL_TMP_DB)) {
      try {
        if (fs.existsSync(DB_FILE)) {
          ...
          fs.writeFileSync(VERCEL_TMP_DB, JSON.stringify({ incidents: [], activityRecords: [] }), 'utf8');
        }
      } catch (e) {}
    }
...
function readDb() {
  try {
    ...
    return { incidents: [], activityRecords: [] };
  }
}
...
app.get('/api/health', (req, res) => {
  res.json({
    status: 'online',
    project: 'ResQNet Disaster Coordination Center',
...
app.get('/api/incidents/:id', (req, res) => {
  const db = readDb();
  const item = (db.incidents || []).find(i => String(i.id) === String(req.params.id));
  if (!item) {
    return res.status(404).json({ error: 'Disaster Incident Report not found' });
  }
  res.json(item);
});
...
app.delete('/api/incidents/:id', (req, res) => {
  ...
  res.json({ message: 'Disaster Incident Report deleted successfully', id: req.params.id });
});
...
if (require.main === module) {
  app.listen(PORT, () => {
    console.log('======================================================');
    console.log('🚀 ResQNet Disaster Coordination Center is running!');
    console.log('📂 Category: Emergency Management');

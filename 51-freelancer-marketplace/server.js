/**
 * GigSphere Freelancer & Client Marketplace - Backend REST API Server
 * Domain: Freelance & Gig Economy
    ...
    if (!fs.existsSync(VERCEL_TMP_DB)) {
      try {
        if (fs.existsSync(DB_FILE)) {
          ...
          fs.writeFileSync(VERCEL_TMP_DB, JSON.stringify({ gigs: [], activityRecords: [] }), 'utf8');
        }
      } catch (e) {}
    }
...
app.get('/api/health', (req, res) => {
  res.json({
    status: 'online',
    project: 'GigSphere Freelancer & Client Marketplace',
...
app.get('/api/gigs/:id', (req, res) => {
  const db = readDb();
  const item = (db.gigs || []).find(i => String(i.id) === String(req.params.id));
  if (!item) {
    return res.status(404).json({ error: 'Gig Project Listing not found' });
  }
  res.json(item);
});
...
app.delete('/api/gigs/:id', (req, res) => {
  ...
  res.json({ message: 'Gig Project Listing deleted successfully', id: req.params.id });
});
...
if (require.main === module) {
  app.listen(PORT, () => {
    console.log('======================================================');
    console.log('🚀 GigSphere Freelancer & Client Marketplace is running!');
    console.log('📂 Category: Freelance & Gig Economy');

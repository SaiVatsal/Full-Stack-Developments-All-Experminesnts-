...
- **Secondary Entity:** `Contract Milestones`
...
1. **Complete RESTful CRUD API:**
   - `GET /api/gigs` — List & search records with multi-field queries.
   - `GET /api/gigs/:id` — Retrieve detailed entity record.
   - `POST /api/gigs` — Create new validated entity with auto-timestamps.
   - `PUT /api/gigs/:id` — Modify existing record with verification.
   - `DELETE /api/gigs/:id` — Remove entity from persistent storage.

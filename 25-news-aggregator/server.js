/**
 * GlobalPulse News Aggregator - Backend REST API Server
 * Domain: News & Media
    ...
    if (!fs.existsSync(VERCEL_TMP_DB)) {
      try {
        if (fs.existsSync(DB_FILE)) {
          ...
          fs.writeFileSync(VERCEL_TMP_DB, JSON.stringify({ articles: [], activityRecords: [] }), 'utf8');
        }
      } catch (e) {}
    }
...
function readDb() {
  try {
    ...
    return { articles: [], activityRecords: [] };
  }
}
...
app.get('/api/articles/:id', (req, res) => {
  const db = readDb();
  const item = (db.articles || []).find(i => String(i.id) === String(req.params.id));
  if (!item) {
    return res.status(404).json({ error: 'News Wire Article not found' });
  }
  res.json(item);
});
...
app.delete('/api/articles/:id', (req, res) => {
  ...
  res.json({ message: 'News Wire Article deleted successfully', id: req.params.id });
});
...
if (require.main === module) {
  app.listen(PORT, () => {
    console.log('======================================================');
    console.log('🚀 GlobalPulse News Aggregator is running!');
    console.log('📂 Category: News & Media');

/**
 * SplitFair Group Expense App - Backend REST API Server
 * Domain: FinTech
    ...
    if (!fs.existsSync(VERCEL_TMP_DB)) {
      try {
        if (fs.existsSync(DB_FILE)) {
          ...
          fs.writeFileSync(VERCEL_TMP_DB, JSON.stringify({ expenses: [], activityRecords: [] }), 'utf8');
        }
      } catch (e) {}
    }
...
function readDb() {
  try {
    ...
    return { expenses: [], activityRecords: [] };
  }
}
...
app.get('/api/health', (req, res) => {
  res.json({
    status: 'online',
    project: 'SplitFair Group Expense App',
    category: 'FinTech',
    student: 'SaiVatsal',
...
app.get('/api/expenses/:id', (req, res) => {
  const db = readDb();
  const item = (db.expenses || []).find(i => String(i.id) === String(req.params.id));
  if (!item) {
    return res.status(404).json({ error: 'Shared Expense Entry not found' });
  }
  res.json(item);
});
...
app.delete('/api/expenses/:id', (req, res) => {
  ...
  res.json({ message: 'Shared Expense Entry deleted successfully', id: req.params.id });
});
...
if (require.main === module) {
  app.listen(PORT, () => {
    console.log('======================================================');
    console.log('🚀 SplitFair Group Expense App is running!');
    console.log('📂 Category: FinTech');

/**
 * DevCraft Masterclass Portfolio - Backend REST API Server
 * Domain: Portfolio & Career
    ...
    if (!fs.existsSync(VERCEL_TMP_DB)) {
      try {
        if (fs.existsSync(DB_FILE)) {
          ...
          fs.writeFileSync(VERCEL_TMP_DB, JSON.stringify({ projects: [], activityRecords: [] }), 'utf8');
        }
      } catch (e) {}
    }
...
function readDb() {
  try {
    ...
    return { projects: [], activityRecords: [] };
  }
}
...
app.get('/api/health', (req, res) => {
  res.json({
    status: 'online',
    project: 'DevCraft Masterclass Portfolio',
...
app.get('/api/projects/:id', (req, res) => {
  const db = readDb();
  const item = (db.projects || []).find(i => String(i.id) === String(req.params.id));
  if (!item) {
    return res.status(404).json({ error: 'Showcase Project not found' });
  }
  res.json(item);
});
...
app.delete('/api/projects/:id', (req, res) => {
  ...
  res.json({ message: 'Showcase Project deleted successfully', id: req.params.id });
});
...
if (require.main === module) {
  app.listen(PORT, () => {
    console.log('======================================================');
    console.log('🚀 DevCraft Masterclass Portfolio is running!');
    console.log('📂 Category: Portfolio & Career');

...
- **Secondary Entity:** `Donor Pledges`
...
1. **Complete RESTful CRUD API:**
   - `GET /api/campaigns` — List & search records with multi-field queries.
   - `GET /api/campaigns/:id` — Retrieve detailed entity record.
   - `POST /api/campaigns` — Create new validated entity with auto-timestamps.
   - `PUT /api/campaigns/:id` — Modify existing record with verification.
   - `DELETE /api/campaigns/:id` — Remove entity from persistent storage.

/**
 * KindredHeart Charitable Giving Platform - Backend REST API Server
 * Domain: Charity & Philanthropy
    ...
    if (!fs.existsSync(VERCEL_TMP_DB)) {
      try {
        if (fs.existsSync(DB_FILE)) {
          ...
          fs.writeFileSync(VERCEL_TMP_DB, JSON.stringify({ campaigns: [], activityRecords: [] }), 'utf8');
        }
      } catch (e) {}
    }
...
function readDb() {
  try {
    ...
    return { campaigns: [], activityRecords: [] };
  }
}
...
app.get('/api/health', (req, res) => {
  res.json({
    status: 'online',
    project: 'KindredHeart Charitable Giving Platform',
...
app.get('/api/campaigns/:id', (req, res) => {
  const db = readDb();
  const item = (db.campaigns || []).find(i => String(i.id) === String(req.params.id));
  if (!item) {
    return res.status(404).json({ error: 'Charity Campaign not found' });
  }
  res.json(item);
});
...
app.delete('/api/campaigns/:id', (req, res) => {
  ...
  res.json({ message: 'Charity Campaign deleted successfully', id: req.params.id });
});
...
if (require.main === module) {
  app.listen(PORT, () => {
    console.log('======================================================');
    console.log('🚀 KindredHeart Charitable Giving Platform is running!');
    console.log('📂 Category: Charity & Philanthropy');

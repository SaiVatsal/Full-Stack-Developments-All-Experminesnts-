/**
 * TutorConnect Academic Mentor Finder - Backend REST API Server
 * Domain: Tutoring & Education
    ...
    if (!fs.existsSync(VERCEL_TMP_DB)) {
      try {
        if (fs.existsSync(DB_FILE)) {
          ...
          fs.writeFileSync(VERCEL_TMP_DB, JSON.stringify({ tutors: [], activityRecords: [] }), 'utf8');
        }
      } catch (e) {}
    }
...
app.get('/api/health', (req, res) => {
  res.json({
    status: 'online',
    project: 'TutorConnect Academic Mentor Finder',
...
app.get('/api/tutors/:id', (req, res) => {
  const db = readDb();
  const item = (db.tutors || []).find(i => String(i.id) === String(req.params.id));
  if (!item) {
    return res.status(404).json({ error: 'Mentor Tutor Profile not found' });
  }
  res.json(item);
});
...
app.delete('/api/tutors/:id', (req, res) => {
  ...
  res.json({ message: 'Mentor Tutor Profile deleted successfully', id: req.params.id });
});
...
if (require.main === module) {
  app.listen(PORT, () => {
    console.log('======================================================');
    console.log('🚀 TutorConnect Academic Mentor Finder is running!');
    console.log('📂 Category: Tutoring & Education');

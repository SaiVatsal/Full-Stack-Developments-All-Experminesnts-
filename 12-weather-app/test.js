...
const server = app.listen(0, async () => {
  ...
  try {
    ...
    console.log(`✔ GET /api/locations: PASSED (${all.data.length} records)`);

    // 3. POST new item
    const postRes = await request('/api/locations', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: {
        city: 'Test Metropolitan City & State',
        country: 'Test Country',
        tempC: 99,
        tempF: 99,
        condition: 'Test Sky Condition',
        humidity: 99,
        windKph: 99,
        uvIndex: 99,
        alert: 'Test Station Advisory'
      }
    });
    ...
    // 4. PUT update item
    const putRes = await request(`/api/locations/${createdId}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
    ...
    // 5. DELETE item
    const delRes = await request(`/api/locations/${createdId}`, {
      method: 'DELETE'
    });
    console.log('✔ DELETE Item:', delRes.status === 200 ? 'PASSED' : 'FAILED');

    console.log(`🎉 All tests passed successfully for ${app.get ? 'AeroCast Meteorological Intelligence' : 'Project'}! Done By SaiVatsal 2500040224`);
    server.close();
    process.exit(0);
  } catch (err) {

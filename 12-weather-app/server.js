/**
 * AeroCast Meteorological Intelligence - Backend REST API Server
    ...
    if (!fs.existsSync(VERCEL_TMP_DB)) {
      try {
        if (fs.existsSync(DB_FILE)) {
          ...
          fs.writeFileSync(VERCEL_TMP_DB, JSON.stringify({ locations: [], forecasts: [] }), 'utf8');
        }
      } catch (e) {}
    }
...
app.get('/api/health', (req, res) => {
  res.json({
    status: 'online',
    project: 'AeroCast Meteorological Intelligence',
...
app.get('/api/locations/:id', (req, res) => {
  const db = readDb();
  const item = (db.locations || []).find(i => String(i.id) === String(req.params.id));
  if (!item) {
    return res.status(404).json({ error: 'Weather Station City not found' });
  }
  res.json(item);
});
...
app.delete('/api/locations/:id', (req, res) => {
  ...
  res.json({ message: 'Weather Station City deleted successfully', id: req.params.id });
});
...
app.post('/api/forecasts', (req, res) => {
  ...
  db.forecasts = db.forecasts || [];
  db.forecasts.unshift(newEntry);
  writeDb(db);
...
if (require.main === module) {
  app.listen(PORT, () => {
    console.log('======================================================');
    console.log('🚀 AeroCast Meteorological Intelligence is running!');
    console.log('📂 Category: Weather & Meteorology');

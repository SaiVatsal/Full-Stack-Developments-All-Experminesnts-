/**
 * SurplusFeed Waste Prevention System - Backend REST API Server
    ...
    if (!fs.existsSync(VERCEL_TMP_DB)) {
      try {
        if (fs.existsSync(DB_FILE)) {
          ...
          fs.writeFileSync(VERCEL_TMP_DB, JSON.stringify({ donations: [], activityRecords: [] }), 'utf8');
        }
      } catch (e) {}
    }
...
function readDb() {
  try {
    ...
    return { donations: [], activityRecords: [] };
  }
}
...
app.get('/api/health', (req, res) => {
  res.json({
    status: 'online',
    project: 'SurplusFeed Waste Prevention System',
...
app.get('/api/donations/:id', (req, res) => {
  const db = readDb();
  const item = (db.donations || []).find(i => String(i.id) === String(req.params.id));
  if (!item) {
    return res.status(404).json({ error: 'Food Surplus Donation not found' });
  }
  res.json(item);
});
...
app.delete('/api/donations/:id', (req, res) => {
  ...
  db.donations = filtered;
  writeDb(db);

  res.json({ message: 'Food Surplus Donation deleted successfully', id: req.params.id });
});
...
if (require.main === module) {
  app.listen(PORT, () => {
    console.log('======================================================');
    console.log('🚀 SurplusFeed Waste Prevention System is running!');
    console.log('📂 Category: Sustainability');

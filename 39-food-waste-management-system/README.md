...
- **Secondary Entity:** `Shelter Claims`
...
1. **Complete RESTful CRUD API:**
   - `GET /api/donations` — List & search records with multi-field queries.
   - `GET /api/donations/:id` — Retrieve detailed entity record.
   - `POST /api/donations` — Create new validated entity with auto-timestamps.
   - `PUT /api/donations/:id` — Modify existing record with verification.
   - `DELETE /api/donations/:id` — Remove entity from persistent storage.

    ...
    if (!fs.existsSync(VERCEL_TMP_DB)) {
      try {
        if (fs.existsSync(DB_FILE)) {
          ...
          fs.writeFileSync(VERCEL_TMP_DB, JSON.stringify({ ventures: [], activityRecords: [] }), 'utf8');
        }
      } catch (e) {}
    }
...
if (require.main === module) {
  app.listen(PORT, () => {
    console.log('======================================================');
    console.log('🚀 VentureForge Crowdfunding Platform is running!');
    console.log('📂 Category: Crowdfunding & Startups');

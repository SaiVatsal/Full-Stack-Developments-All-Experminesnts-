...
- **Secondary Entity:** `Investor Backer Pledges`
...
1. **Complete RESTful CRUD API:**
   - `GET /api/ventures` — List & search records with multi-field queries.
   - `GET /api/ventures/:id` — Retrieve detailed entity record.
   - `POST /api/ventures` — Create new validated entity with auto-timestamps.
   - `PUT /api/ventures/:id` — Modify existing record with verification.
   - `DELETE /api/ventures/:id` — Remove entity from persistent storage.

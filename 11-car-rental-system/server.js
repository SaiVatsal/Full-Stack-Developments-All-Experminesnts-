/**
 * Velocity Auto Fleet & Rental System - Backend REST API Server
 * Domain: Automotive & Travel
    ...
    if (!fs.existsSync(VERCEL_TMP_DB)) {
      try {
        if (fs.existsSync(DB_FILE)) {
          ...
          fs.writeFileSync(VERCEL_TMP_DB, JSON.stringify({ vehicles: [], reservations: [] }), 'utf8');
        }
      } catch (e) {}
    }
...
app.get('/api/stats', (req, res) => {
  ...
  const secondary = db.reservations || [];
  res.json({
...
app.get('/api/vehicles/:id', (req, res) => {
  const db = readDb();
  const item = (db.vehicles || []).find(i => String(i.id) === String(req.params.id));
  if (!item) {
...
app.delete('/api/vehicles/:id', (req, res) => {
  ...
  res.json({ message: 'Rental Vehicle deleted successfully', id: req.params.id });
});
...
if (require.main === module) {
  app.listen(PORT, () => {
    console.log('======================================================');
    console.log('🚀 Velocity Auto Fleet & Rental System is running!');
    console.log('📂 Category: Automotive & Travel');

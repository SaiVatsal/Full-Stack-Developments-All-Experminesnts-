/**
 * PulseNet Community Platform - Backend REST API Server
    ...
    if (!fs.existsSync(VERCEL_TMP_DB)) {
      try {
        if (fs.existsSync(DB_FILE)) {
          ...
          fs.writeFileSync(VERCEL_TMP_DB, JSON.stringify({ posts: [], comments: [] }), 'utf8');
        }
      } catch (e) {}
    }
...
app.get('/api/posts/:id', (req, res) => {
  const db = readDb();
  const item = (db.posts || []).find(i => String(i.id) === String(req.params.id));
  if (!item) {
...
app.delete('/api/posts/:id', (req, res) => {
  ...
  res.json({ message: 'Social Post deleted successfully', id: req.params.id });
});
...
if (require.main === module) {
  app.listen(PORT, () => {
    console.log('======================================================');
    console.log('🚀 PulseNet Community Platform is running!');

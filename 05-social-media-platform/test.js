 ...
 * Project: PulseNet Community Platform
...
const server = app.listen(0, async () => {
  ...
  try {
    ...
    // 3. GET all
    const all = await request('/api/posts');
    console.log(`✔ GET /api/posts: PASSED (${all.data.length} records)`);
    ...
    const postRes = await request('/api/posts', {
      ...
      headers: { 'Content-Type': 'application/json' },
      body: {
        author: 'Test User / Author Name',
        handle: 'Test Username Handle',
        content: 'Test Post Content Body',
        tag: 'Test Topic Tag',
        likes: 99,
        comments: 99,
        shares: 99,
        time: 'Test Post Timestamp'
      }
    });
    ...
    // 5. PUT update item
    const putRes = await request(`/api/posts/${createdId}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
    ...
    // 6. DELETE item
    const delRes = await request(`/api/posts/${createdId}`, {
      method: 'DELETE'
    });
    console.log('✔ DELETE Item:', delRes.status === 200 ? 'PASSED' : 'FAILED');

    console.log(`🎉 All tests passed successfully for ${app.get ? 'PulseNet Community Platform' : 'Project'}! Done By SaiVatsal 2500040224`);
    server.close();
    process.exit(0);
  } catch (err) {

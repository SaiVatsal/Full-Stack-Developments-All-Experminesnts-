...
- **Secondary Entity:** `Auction Live Bids`
...
1. **Complete RESTful CRUD API:**
   - `GET /api/artworks` — List & search records with multi-field queries.
   - `GET /api/artworks/:id` — Retrieve detailed entity record.
   - `POST /api/artworks` — Create new validated entity with auto-timestamps.
   - `PUT /api/artworks/:id` — Modify existing record with verification.
   - `DELETE /api/artworks/:id` — Remove entity from persistent storage.

...
const server = app.listen(0, async () => {
  ...
  try {
    ...
    // 2. GET all
    const all = await request('/api/artworks');
    console.log(`✔ GET /api/artworks: PASSED (${all.data.length} records)`);

    // 3. POST new item
    const postRes = await request('/api/artworks', {
      method: 'POST',
    ...
    // 4. PUT update item
    const putRes = await request(`/api/artworks/${createdId}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
    ...
    // 5. DELETE item
    const delRes = await request(`/api/artworks/${createdId}`, {
      method: 'DELETE'
    });
    console.log('✔ DELETE Item:', delRes.status === 200 ? 'PASSED' : 'FAILED');

    console.log(`🎉 All tests passed successfully for ${app.get ? 'Lumina Online Art & Auction Gallery' : 'Project'}! Done By SaiVatsal 2500040224`);
    server.close();
    process.exit(0);
  } catch (err) {

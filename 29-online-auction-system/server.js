/**
 * BidMaster Real-Time Auction Hub - Backend REST API Server
 * Domain: Auctions & Commerce
    ...
    if (!fs.existsSync(VERCEL_TMP_DB)) {
      try {
        if (fs.existsSync(DB_FILE)) {
          ...
          fs.writeFileSync(VERCEL_TMP_DB, JSON.stringify({ auctions: [], activityRecords: [] }), 'utf8');
        }
      } catch (e) {}
    }
...
function readDb() {
  try {
    ...
    return { auctions: [], activityRecords: [] };
  }
}
...
app.get('/api/health', (req, res) => {
  res.json({
    status: 'online',
    project: 'BidMaster Real-Time Auction Hub',
    category: 'Auctions & Commerce',
    student: 'SaiVatsal',
...
app.get('/api/auctions/:id', (req, res) => {
  const db = readDb();
  const item = (db.auctions || []).find(i => String(i.id) === String(req.params.id));
  if (!item) {
...
app.delete('/api/auctions/:id', (req, res) => {
  ...
  db.auctions = filtered;
  writeDb(db);

  res.json({ message: 'Auction Listing deleted successfully', id: req.params.id });
});
...
if (require.main === module) {
  app.listen(PORT, () => {
    console.log('======================================================');
    console.log('🚀 BidMaster Real-Time Auction Hub is running!');
    console.log('📂 Category: Auctions & Commerce');

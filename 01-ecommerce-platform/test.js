...
const server = app.listen(0, async () => {
  ...
  try {
    ...
    // 3. GET all
    const all = await request('/api/products');
    console.log(`✔ GET /api/products: PASSED (${all.data.length} records)`);
    ...
    const postRes = await request('/api/products', {
      ...
      body: {
        ...
        stock: 99,
        rating: 99,
        status: 'Test Stock Status',
        description: 'Test Product Overview'
      }
    });
    ...
    // 5. PUT update item
    const putRes = await request(`/api/products/${createdId}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
    ...
    console.log(`🎉 All tests passed successfully for ${app.get ? 'Nexus E-Commerce Platform' : 'Project'}! Done By SaiVatsal 2500040224`);
    server.close();
    process.exit(0);
  } catch (err) {

/**
 * Nexus E-Commerce Platform - Backend REST API Server
    ...
    if (!fs.existsSync(VERCEL_TMP_DB)) {
      try {
        if (fs.existsSync(DB_FILE)) {
          ...
          fs.writeFileSync(VERCEL_TMP_DB, JSON.stringify({ products: [], orders: [] }), 'utf8');
        }
      } catch (e) {}
    }
...
app.get('/api/products/:id', (req, res) => {
  const db = readDb();
  const item = (db.products || []).find(i => String(i.id) === String(req.params.id));
  if (!item) {
...
if (require.main === module) {
  app.listen(PORT, () => {
    console.log('======================================================');
    console.log('🚀 Nexus E-Commerce Platform is running!');
    console.log('📂 Category: E-Commerce & Retail');

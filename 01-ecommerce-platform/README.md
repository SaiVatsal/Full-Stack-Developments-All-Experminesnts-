...
- **Secondary Entity:** `Customer Orders`
...
1. **Complete RESTful CRUD API:**
   - `GET /api/products` — List & search records with multi-field queries.
   - `GET /api/products/:id` — Retrieve detailed entity record.
   - `POST /api/products` — Create new validated entity with auto-timestamps.
   - `PUT /api/products/:id` — Modify existing record with verification.
   - `DELETE /api/products/:id` — Remove entity from persistent storage.
   - `GET /api/orders` — Secondary domain service integration.

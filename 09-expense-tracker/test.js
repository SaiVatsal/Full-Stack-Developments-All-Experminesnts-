...
const server = app.listen(0, async () => {
  ...
  try {
    ...
    // 2. GET all
    const all = await request('/api/transactions');
    console.log(`✔ GET /api/transactions: PASSED (${all.data.length} records)`);

    // 3. POST new item
    const postRes = await request('/api/transactions', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: {
        title: 'Test Transaction Description',
        category: 'Test Budget Category',
        amount: 99,
        type: 'Test Cash Flow Direction',
        account: 'Test Payment Account',
        date: 'Test Transaction Date',
        status: 'Test Clearance Status',
        loggedBy: 'Test Auditor Name'
      }
    });
    ...
    // 4. PUT update item
    const putRes = await request(`/api/transactions/${createdId}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
    ...
    // 5. DELETE item
    const delRes = await request(`/api/transactions/${createdId}`, {
      method: 'DELETE'
    });
    console.log('✔ DELETE Item:', delRes.status === 200 ? 'PASSED' : 'FAILED');

    console.log(`🎉 All tests passed successfully for ${app.get ? 'SmartLedger Personal Expense Tracker' : 'Project'}! Done By SaiVatsal 2500040224`);
    server.close();
    process.exit(0);
  } catch (err) {

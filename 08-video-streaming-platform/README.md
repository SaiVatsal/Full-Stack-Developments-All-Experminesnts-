...
- **Secondary Entity:** `Subscriber Watchlist Queue`
...
1. **Complete RESTful CRUD API:**
   - `GET /api/videos` — List & search records with multi-field queries.
   - `GET /api/videos/:id` — Retrieve detailed entity record.
   - `POST /api/videos` — Create new validated entity with auto-timestamps.
   - `PUT /api/videos/:id` — Modify existing record with verification.
   - `DELETE /api/videos/:id` — Remove entity from persistent storage.
   - `GET /api/watchlist` — Secondary domain service integration.

/**
 * StreamCrest Cinema & Video Hub - Backend REST API Server
 * Domain: Digital Media & Streaming
    ...
    if (!fs.existsSync(VERCEL_TMP_DB)) {
      try {
        if (fs.existsSync(DB_FILE)) {
          ...
          fs.writeFileSync(VERCEL_TMP_DB, JSON.stringify({ videos: [], watchlist: [] }), 'utf8');
        }
      } catch (e) {}
    }
...
app.get('/api/videos/:id', (req, res) => {
  const db = readDb();
  const item = (db.videos || []).find(i => String(i.id) === String(req.params.id));
  if (!item) {
    return res.status(404).json({ error: 'Video Title not found' });
  }
  res.json(item);
});
...
app.delete('/api/videos/:id', (req, res) => {
  ...
  res.json({ message: 'Video Title deleted successfully', id: req.params.id });
});
...
if (require.main === module) {
  app.listen(PORT, () => {
    console.log('======================================================');
    console.log('🚀 StreamCrest Cinema & Video Hub is running!');
    console.log('📂 Category: Digital Media & Streaming');

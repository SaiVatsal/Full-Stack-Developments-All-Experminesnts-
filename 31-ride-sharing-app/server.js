/**
 * SwiftRide Mobility Platform - Backend REST API Server
    ...
    if (!fs.existsSync(VERCEL_TMP_DB)) {
      try {
        if (fs.existsSync(DB_FILE)) {
          ...
          fs.writeFileSync(VERCEL_TMP_DB, JSON.stringify({ rides: [], activityRecords: [] }), 'utf8');
        }
      } catch (e) {}
    }
...
app.get('/api/health', (req, res) => {
  res.json({
    status: 'online',
    project: 'SwiftRide Mobility Platform',
...
app.get('/api/rides/:id', (req, res) => {
  const db = readDb();
  const item = (db.rides || []).find(i => String(i.id) === String(req.params.id));
  if (!item) {
    return res.status(404).json({ error: 'Ride Route Request not found' });
  }
  res.json(item);
});
...
app.delete('/api/rides/:id', (req, res) => {
  ...
  res.json({ message: 'Ride Route Request deleted successfully', id: req.params.id });
});
...
if (require.main === module) {
  app.listen(PORT, () => {
    console.log('======================================================');
    console.log('🚀 SwiftRide Mobility Platform is running!');

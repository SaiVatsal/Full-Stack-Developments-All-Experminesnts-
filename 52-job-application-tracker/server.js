/**
 * CareerPulse Job Application Tracker - Backend REST API Server
    ...
    if (!fs.existsSync(VERCEL_TMP_DB)) {
      try {
        if (fs.existsSync(DB_FILE)) {
          ...
          fs.writeFileSync(VERCEL_TMP_DB, JSON.stringify({ applications: [], activityRecords: [] }), 'utf8');
        }
      } catch (e) {}
    }
...
function readDb() {
  try {
    ...
    return { applications: [], activityRecords: [] };
  }
}
...
app.get('/api/health', (req, res) => {
  res.json({
    status: 'online',
    project: 'CareerPulse Job Application Tracker',
...
app.get('/api/applications/:id', (req, res) => {
  const db = readDb();
  const item = (db.applications || []).find(i => String(i.id) === String(req.params.id));
  if (!item) {
...
app.delete('/api/applications/:id', (req, res) => {
  ...
  res.json({ message: 'Job Application deleted successfully', id: req.params.id });
});
...
if (require.main === module) {
  app.listen(PORT, () => {
    console.log('======================================================');
    console.log('🚀 CareerPulse Job Application Tracker is running!');
    console.log('📂 Category: Career Management');

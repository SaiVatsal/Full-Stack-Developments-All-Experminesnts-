/**
 * ScholarPeer Academic Journal Workflow - Backend REST API Server
    ...
    if (!fs.existsSync(VERCEL_TMP_DB)) {
      try {
        if (fs.existsSync(DB_FILE)) {
          ...
          fs.writeFileSync(VERCEL_TMP_DB, JSON.stringify({ manuscripts: [], activityRecords: [] }), 'utf8');
        }
      } catch (e) {}
    }
...
function readDb() {
  try {
    ...
    return { manuscripts: [], activityRecords: [] };
  }
}
...
app.get('/api/health', (req, res) => {
  res.json({
    status: 'online',
    project: 'ScholarPeer Academic Journal Workflow',
...
app.get('/api/manuscripts/:id', (req, res) => {
  const db = readDb();
  const item = (db.manuscripts || []).find(i => String(i.id) === String(req.params.id));
  if (!item) {
    return res.status(404).json({ error: 'Manuscript Paper not found' });
  }
  res.json(item);
});
...
app.delete('/api/manuscripts/:id', (req, res) => {
  ...
  db.manuscripts = filtered;
  writeDb(db);

  res.json({ message: 'Manuscript Paper deleted successfully', id: req.params.id });
});
...
if (require.main === module) {
  app.listen(PORT, () => {
    console.log('======================================================');
    console.log('🚀 ScholarPeer Academic Journal Workflow is running!');
    console.log('📂 Category: Academic Research');

...
- **Secondary Entity:** `Peer Review Decisions`
...
1. **Complete RESTful CRUD API:**
   - `GET /api/manuscripts` — List & search records with multi-field queries.
   - `GET /api/manuscripts/:id` — Retrieve detailed entity record.
   - `POST /api/manuscripts` — Create new validated entity with auto-timestamps.
   - `PUT /api/manuscripts/:id` — Modify existing record with verification.
   - `DELETE /api/manuscripts/:id` — Remove entity from persistent storage.

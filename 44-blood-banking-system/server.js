/**
 * LifeDrop Blood Bank Registry - Backend REST API Server
 * Domain: Medical & Blood Banking
    ...
    if (!fs.existsSync(VERCEL_TMP_DB)) {
      try {
        if (fs.existsSync(DB_FILE)) {
          ...
          fs.writeFileSync(VERCEL_TMP_DB, JSON.stringify({ bloodUnits: [], activityRecords: [] }), 'utf8');
        }
      } catch (e) {}
    }
...
function readDb() {
  try {
    ...
    return { bloodUnits: [], activityRecords: [] };
  }
}
...
app.get('/api/health', (req, res) => {
  res.json({
    status: 'online',
    project: 'LifeDrop Blood Bank Registry',
    category: 'Medical & Blood Banking',
    student: 'SaiVatsal',
...
app.get('/api/bloodUnits', (req, res) => {
  const db = readDb();
  let items = db.bloodUnits || [];
...
app.get('/api/bloodUnits/:id', (req, res) => {
  const db = readDb();
  const item = (db.bloodUnits || []).find(i => String(i.id) === String(req.params.id));
  if (!item) {
    return res.status(404).json({ error: 'Blood Unit Stock not found' });
  }
  res.json(item);
});

// 5. POST new primary entity
app.post('/api/bloodUnits', (req, res) => {
  const db = readDb();
  const items = db.bloodUnits || [];
...
app.put('/api/bloodUnits/:id', (req, res) => {
  const db = readDb();
  const items = db.bloodUnits || [];
...
app.delete('/api/bloodUnits/:id', (req, res) => {
  const db = readDb();
  const items = db.bloodUnits || [];
  ...
  db.bloodUnits = filtered;
  writeDb(db);

  res.json({ message: 'Blood Unit Stock deleted successfully', id: req.params.id });
});
...
if (require.main === module) {
  app.listen(PORT, () => {
    console.log('======================================================');
    console.log('🚀 LifeDrop Blood Bank Registry is running!');
    console.log('📂 Category: Medical & Blood Banking');

...
- **Secondary Entity:** `Emergency Hospital Requests`
...
1. **Complete RESTful CRUD API:**
   - `GET /api/bloodUnits` — List & search records with multi-field queries.
   - `GET /api/bloodUnits/:id` — Retrieve detailed entity record.
   - `POST /api/bloodUnits` — Create new validated entity with auto-timestamps.
   - `PUT /api/bloodUnits/:id` — Modify existing record with verification.
   - `DELETE /api/bloodUnits/:id` — Remove entity from persistent storage.

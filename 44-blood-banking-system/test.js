...
const server = app.listen(0, async () => {
  ...
  try {
    ...
    // 2. GET all
    const all = await request('/api/bloodUnits');
    console.log(`✔ GET /api/bloodUnits: PASSED (${all.data.length} records)`);

    // 3. POST new item
    const postRes = await request('/api/bloodUnits', {
      method: 'POST',
    ...
    // 4. PUT update item
    const putRes = await request(`/api/bloodUnits/${createdId}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
    ...
    // 5. DELETE item
    const delRes = await request(`/api/bloodUnits/${createdId}`, {
      method: 'DELETE'
    });
    console.log('✔ DELETE Item:', delRes.status === 200 ? 'PASSED' : 'FAILED');

    console.log(`🎉 All tests passed successfully for ${app.get ? 'LifeDrop Blood Bank Registry' : 'Project'}! Done By SaiVatsal 2500040224`);
    server.close();
    process.exit(0);
  } catch (err) {

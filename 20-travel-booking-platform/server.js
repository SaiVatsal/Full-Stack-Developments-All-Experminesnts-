/**
 * WanderLust Global Travel & Flight Booking - Backend REST API Server
    ...
    if (!fs.existsSync(VERCEL_TMP_DB)) {
      try {
        if (fs.existsSync(DB_FILE)) {
          ...
          fs.writeFileSync(VERCEL_TMP_DB, JSON.stringify({ packages: [], bookings: [] }), 'utf8');
        }
      } catch (e) {}
    }
...
app.get('/api/packages/:id', (req, res) => {
  const db = readDb();
  const item = (db.packages || []).find(i => String(i.id) === String(req.params.id));
  if (!item) {
...
app.delete('/api/packages/:id', (req, res) => {
  ...
  res.json({ message: 'Travel Package & Flight deleted successfully', id: req.params.id });
});

/**
 * KisanDirect Farmer-Buyer Marketplace - Backend REST API Server
    ...
    if (!fs.existsSync(VERCEL_TMP_DB)) {
      try {
        if (fs.existsSync(DB_FILE)) {
          ...
          fs.writeFileSync(VERCEL_TMP_DB, JSON.stringify({ crops: [], activityRecords: [] }), 'utf8');
        }
      } catch (e) {}
    }
...
app.get('/api/health', (req, res) => {
  res.json({
    status: 'online',
    project: 'KisanDirect Farmer-Buyer Marketplace',
...
app.get('/api/crops/:id', (req, res) => {
  const db = readDb();
  const item = (db.crops || []).find(i => String(i.id) === String(req.params.id));
  if (!item) {
    return res.status(404).json({ error: 'Crop Harvest Lot not found' });
  }
  res.json(item);
});
...
app.delete('/api/crops/:id', (req, res) => {
  ...
  res.json({ message: 'Crop Harvest Lot deleted successfully', id: req.params.id });
});
...
if (require.main === module) {
  app.listen(PORT, () => {
    console.log('======================================================');
    console.log('🚀 KisanDirect Farmer-Buyer Marketplace is running!');
    console.log('📂 Category: Agriculture & AgriTech');

...
- **Secondary Entity:** `Module Progress`
...
1. **Complete RESTful CRUD API:**
   - `GET /api/resources` — List & search records with multi-field queries.
   - `GET /api/resources/:id` — Retrieve detailed entity record.
   - `POST /api/resources` — Create new validated entity with auto-timestamps.
   - `PUT /api/resources/:id` — Modify existing record with verification.
   - `DELETE /api/resources/:id` — Remove entity from persistent storage.

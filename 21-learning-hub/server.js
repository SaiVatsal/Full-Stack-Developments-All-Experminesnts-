/**
 * SkillCore Student Learning Hub - Backend REST API Server
 * Domain: Education
    ...
    if (!fs.existsSync(VERCEL_TMP_DB)) {
      try {
        if (fs.existsSync(DB_FILE)) {
          ...
          fs.writeFileSync(VERCEL_TMP_DB, JSON.stringify({ resources: [], activityRecords: [] }), 'utf8');
        }
      } catch (e) {}
    }
...
function readDb() {
  try {
    ...
    return { resources: [], activityRecords: [] };
  }
}
...
app.get('/api/health', (req, res) => {
  res.json({
    status: 'online',
    project: 'SkillCore Student Learning Hub',
    category: 'Education',
    student: 'SaiVatsal',
...
app.get('/api/resources/:id', (req, res) => {
  const db = readDb();
  const item = (db.resources || []).find(i => String(i.id) === String(req.params.id));
  if (!item) {
    return res.status(404).json({ error: 'Resource & Lesson not found' });
  }
  res.json(item);
});
...
app.delete('/api/resources/:id', (req, res) => {
  ...
  db.resources = filtered;
  writeDb(db);

  res.json({ message: 'Resource & Lesson deleted successfully', id: req.params.id });
});
...
if (require.main === module) {
  app.listen(PORT, () => {
    console.log('======================================================');
    console.log('🚀 SkillCore Student Learning Hub is running!');

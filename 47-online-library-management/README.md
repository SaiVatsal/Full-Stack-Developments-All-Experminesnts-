...
- **Secondary Entity:** `Member Borrow History`
...
1. **Complete RESTful CRUD API:**
   - `GET /api/books` — List & search records with multi-field queries.
   - `GET /api/books/:id` — Retrieve detailed entity record.
   - `POST /api/books` — Create new validated entity with auto-timestamps.
   - `PUT /api/books/:id` — Modify existing record with verification.
   - `DELETE /api/books/:id` — Remove entity from persistent storage.

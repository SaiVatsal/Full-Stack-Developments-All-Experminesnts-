/**
 * BiblioTech Library Management System - Backend REST API Server
 * Domain: Library & Archival
    ...
    if (!fs.existsSync(VERCEL_TMP_DB)) {
      try {
        if (fs.existsSync(DB_FILE)) {
          ...
          fs.writeFileSync(VERCEL_TMP_DB, JSON.stringify({ books: [], activityRecords: [] }), 'utf8');
        }
      } catch (e) {}
    }
...
app.get('/api/health', (req, res) => {
  res.json({
    status: 'online',
    project: 'BiblioTech Library Management System',
    category: 'Library & Archival',
    student: 'SaiVatsal',
...
app.get('/api/books/:id', (req, res) => {
  const db = readDb();
  const item = (db.books || []).find(i => String(i.id) === String(req.params.id));
  if (!item) {
    return res.status(404).json({ error: 'Catalog Book Entry not found' });
  }
  res.json(item);
});
...
app.delete('/api/books/:id', (req, res) => {
  ...
  res.json({ message: 'Catalog Book Entry deleted successfully', id: req.params.id });
});
...
if (require.main === module) {
  app.listen(PORT, () => {
    console.log('======================================================');
    console.log('🚀 BiblioTech Library Management System is running!');
    console.log('📂 Category: Library & Archival');

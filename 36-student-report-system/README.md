...
- **Secondary Entity:** `Subject Marks Ledger`
...
1. **Complete RESTful CRUD API:**
   - `GET /api/reports` — List & search records with multi-field queries.
   - `GET /api/reports/:id` — Retrieve detailed entity record.
   - `POST /api/reports` — Create new validated entity with auto-timestamps.
   - `PUT /api/reports/:id` — Modify existing record with verification.
   - `DELETE /api/reports/:id` — Remove entity from persistent storage.

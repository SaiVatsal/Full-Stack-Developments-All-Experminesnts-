...
const server = app.listen(0, async () => {
  ...
  try {
    ...
    console.log(`✔ GET /api/reports: PASSED (${all.data.length} records)`);

    // 3. POST new item
    const postRes = await request('/api/reports', {
      method: 'POST',
    ...
    // 4. PUT update item
    const putRes = await request(`/api/reports/${createdId}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
    ...
    // 5. DELETE item
    const delRes = await request(`/api/reports/${createdId}`, {
      method: 'DELETE'
    });
    console.log('✔ DELETE Item:', delRes.status === 200 ? 'PASSED' : 'FAILED');

    console.log(`🎉 All tests passed successfully for ${app.get ? 'AcademiaPlus Student Report System' : 'Project'}! Done By SaiVatsal 2500040224`);
    server.close();
    process.exit(0);
  } catch (err) {

/**
 * AcademiaPlus Student Report System - Backend REST API Server
 * Domain: Academic ERP
    ...
    if (!fs.existsSync(VERCEL_TMP_DB)) {
      try {
        if (fs.existsSync(DB_FILE)) {
          ...
          fs.writeFileSync(VERCEL_TMP_DB, JSON.stringify({ reports: [], activityRecords: [] }), 'utf8');
        }
      } catch (e) {}
    }
...
function readDb() {
  try {
    ...
    return { reports: [], activityRecords: [] };
  }
}
...
app.get('/api/health', (req, res) => {
  res.json({
    status: 'online',
    project: 'AcademiaPlus Student Report System',
...
app.get('/api/reports/:id', (req, res) => {
  const db = readDb();
  const item = (db.reports || []).find(i => String(i.id) === String(req.params.id));
  if (!item) {
    return res.status(404).json({ error: 'Student Grade Report not found' });
  }
  res.json(item);
});
...
app.delete('/api/reports/:id', (req, res) => {
  ...
  res.json({ message: 'Student Grade Report deleted successfully', id: req.params.id });
});
...
if (require.main === module) {
  app.listen(PORT, () => {
    console.log('======================================================');
    console.log('🚀 AcademiaPlus Student Report System is running!');
    console.log('📂 Category: Academic ERP');

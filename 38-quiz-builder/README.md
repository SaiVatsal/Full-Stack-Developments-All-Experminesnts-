...
- **Secondary Entity:** `Candidate Scores`
...
1. **Complete RESTful CRUD API:**
   - `GET /api/quizzes` — List & search records with multi-field queries.
   - `GET /api/quizzes/:id` — Retrieve detailed entity record.
   - `POST /api/quizzes` — Create new validated entity with auto-timestamps.
   - `PUT /api/quizzes/:id` — Modify existing record with verification.
   - `DELETE /api/quizzes/:id` — Remove entity from persistent storage.

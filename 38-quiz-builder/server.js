/**
 * QuizCraft Assessment Builder - Backend REST API Server
 * Domain: E-Learning
    ...
    if (!fs.existsSync(VERCEL_TMP_DB)) {
      try {
        if (fs.existsSync(DB_FILE)) {
          ...
          fs.writeFileSync(VERCEL_TMP_DB, JSON.stringify({ quizzes: [], activityRecords: [] }), 'utf8');
        }
      } catch (e) {}
    }
...
function readDb() {
  try {
    ...
    return { quizzes: [], activityRecords: [] };
  }
}
...
app.get('/api/health', (req, res) => {
  res.json({
    status: 'online',
    project: 'QuizCraft Assessment Builder',
    category: 'E-Learning',
    student: 'SaiVatsal',
...
app.get('/api/quizzes/:id', (req, res) => {
  const db = readDb();
  const item = (db.quizzes || []).find(i => String(i.id) === String(req.params.id));
  if (!item) {
    return res.status(404).json({ error: 'Interactive Quiz Assessment not found' });
  }
  res.json(item);
});
...
app.delete('/api/quizzes/:id', (req, res) => {
  ...
  res.json({ message: 'Interactive Quiz Assessment deleted successfully', id: req.params.id });
});
...
if (require.main === module) {
  app.listen(PORT, () => {
    console.log('======================================================');
    console.log('🚀 QuizCraft Assessment Builder is running!');
    console.log('📂 Category: E-Learning');

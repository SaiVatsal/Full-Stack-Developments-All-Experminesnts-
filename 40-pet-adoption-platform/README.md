...
- **Secondary Entity:** `Adoption Applications`
...
1. **Complete RESTful CRUD API:**
   - `GET /api/pets` — List & search records with multi-field queries.
   - `GET /api/pets/:id` — Retrieve detailed entity record.
   - `POST /api/pets` — Create new validated entity with auto-timestamps.
   - `PUT /api/pets/:id` — Modify existing record with verification.
   - `DELETE /api/pets/:id` — Remove entity from persistent storage.

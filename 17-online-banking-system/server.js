/**
 * AuraBank Educational Digital Banking Suite - Backend REST API Server
 * Domain: FinTech & Banking Simulation
    ...
    if (!fs.existsSync(VERCEL_TMP_DB)) {
      try {
        if (fs.existsSync(DB_FILE)) {
          ...
          fs.writeFileSync(VERCEL_TMP_DB, JSON.stringify({ accounts: [], transfers: [] }), 'utf8');
        }
      } catch (e) {}
    }
...
app.get('/api/accounts/:id', (req, res) => {
  const db = readDb();
  const item = (db.accounts || []).find(i => String(i.id) === String(req.params.id));
  if (!item) {
    return res.status(404).json({ error: 'Bank Account & Customer not found' });
  }
  res.json(item);
});
...
app.delete('/api/accounts/:id', (req, res) => {
  ...
  db.accounts = filtered;
  writeDb(db);

  res.json({ message: 'Bank Account & Customer deleted successfully', id: req.params.id });
});
...
if (require.main === module) {
  app.listen(PORT, () => {
    console.log('======================================================');
    console.log('🚀 AuraBank Educational Digital Banking Suite is running!');
    console.log('📂 Category: FinTech & Banking Simulation');

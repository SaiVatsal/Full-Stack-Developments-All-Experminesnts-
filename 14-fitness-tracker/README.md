...
- **Secondary Entity:** `Daily Nutrition & Macro Log`
...
1. **Complete RESTful CRUD API:**
   - `GET /api/workouts` — List & search records with multi-field queries.
   - `GET /api/workouts/:id` — Retrieve detailed entity record.
   - `POST /api/workouts` — Create new validated entity with auto-timestamps.
   - `PUT /api/workouts/:id` — Modify existing record with verification.
   - `DELETE /api/workouts/:id` — Remove entity from persistent storage.
   - `GET /api/nutrition` — Secondary domain service integration.

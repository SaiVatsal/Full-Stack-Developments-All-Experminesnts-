/**
 * FitPulse Athletic & Nutrition Suite - Backend REST API Server
    ...
    if (!fs.existsSync(VERCEL_TMP_DB)) {
      try {
        if (fs.existsSync(DB_FILE)) {
          ...
          fs.writeFileSync(VERCEL_TMP_DB, JSON.stringify({ workouts: [], nutrition: [] }), 'utf8');
        }
      } catch (e) {}
    }
...
app.get('/api/health', (req, res) => {
  res.json({
    status: 'online',
    project: 'FitPulse Athletic & Nutrition Suite',
...
app.get('/api/stats', (req, res) => {
  const db = readDb();
  const items = db.workouts || [];
  const secondary = db.nutrition || [];
  res.json({
...
app.get('/api/workouts/:id', (req, res) => {
  const db = readDb();
  const item = (db.workouts || []).find(i => String(i.id) === String(req.params.id));
  if (!item) {
    return res.status(404).json({ error: 'Workout & Activity Log not found' });
  }
  res.json(item);
});
...
app.delete('/api/workouts/:id', (req, res) => {
  ...
  db.workouts = filtered;
  writeDb(db);

  res.json({ message: 'Workout & Activity Log deleted successfully', id: req.params.id });
});
...
if (require.main === module) {
  app.listen(PORT, () => {
    console.log('======================================================');
    console.log('🚀 FitPulse Athletic & Nutrition Suite is running!');
    console.log('📂 Category: Health & Fitness');

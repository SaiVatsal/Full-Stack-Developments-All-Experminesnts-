...
const server = app.listen(0, async () => {
  ...
  try {
    ...
    // 3. GET all
    const all = await request('/api/budgetCaps');
    console.log(`✔ GET /api/budgetCaps: PASSED (${all.data.length} records)`);

    // 4. POST new item
    const postRes = await request('/api/budgetCaps', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: {
    ...
    // 5. PUT update item
    const putRes = await request(`/api/budgetCaps/${createdId}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
    ...
    // 6. DELETE item
    const delRes = await request(`/api/budgetCaps/${createdId}`, {
      method: 'DELETE'
    });
    console.log('✔ DELETE Item:', delRes.status === 200 ? 'PASSED' : 'FAILED');

    console.log(`🎉 All tests passed successfully for ${app.get ? 'CapitalWise Personal Budget Planner' : 'Project'}! Done By SaiVatsal 2500040224`);
    server.close();
    process.exit(0);
  } catch (err) {

/**
 * CapitalWise Personal Budget Planner - Backend REST API Server
    ...
    if (!fs.existsSync(VERCEL_TMP_DB)) {
      try {
        if (fs.existsSync(DB_FILE)) {
          ...
          fs.writeFileSync(VERCEL_TMP_DB, JSON.stringify({ budgetCaps: [], activityRecords: [] }), 'utf8');
        }
      } catch (e) {}
    }
...
function readDb() {
  try {
    ...
    return { budgetCaps: [], activityRecords: [] };
  }
}
...
app.get('/api/health', (req, res) => {
  res.json({
    status: 'online',
    project: 'CapitalWise Personal Budget Planner',
...
app.get('/api/budgetCaps', (req, res) => {
  const db = readDb();
  let items = db.budgetCaps || [];
...
app.get('/api/budgetCaps/:id', (req, res) => {
  const db = readDb();
  const item = (db.budgetCaps || []).find(i => String(i.id) === String(req.params.id));
  if (!item) {
    return res.status(404).json({ error: 'Category Budget Cap not found' });
  }
  res.json(item);
});

// 5. POST new primary entity
app.post('/api/budgetCaps', (req, res) => {
  const db = readDb();
  const items = db.budgetCaps || [];
...
app.put('/api/budgetCaps/:id', (req, res) => {
  const db = readDb();
  const items = db.budgetCaps || [];
...
app.delete('/api/budgetCaps/:id', (req, res) => {
  const db = readDb();
  const items = db.budgetCaps || [];
  ...
  db.budgetCaps = filtered;
  writeDb(db);

  res.json({ message: 'Category Budget Cap deleted successfully', id: req.params.id });
});
...
if (require.main === module) {
  app.listen(PORT, () => {
    console.log('======================================================');
    console.log('🚀 CapitalWise Personal Budget Planner is running!');
    console.log('📂 Category: Personal Finance');

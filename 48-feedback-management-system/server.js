    ...
    if (!fs.existsSync(VERCEL_TMP_DB)) {
      try {
        if (fs.existsSync(DB_FILE)) {
          ...
          fs.writeFileSync(VERCEL_TMP_DB, JSON.stringify({ surveys: [], activityRecords: [] }), 'utf8');
        }
      } catch (e) {}
    }
...
if (require.main === module) {
  app.listen(PORT, () => {
    console.log('======================================================');
    console.log('🚀 VoiceMetrics Enterprise Feedback Hub is running!');
    console.log('📂 Category: Customer Intelligence');

...
- **Secondary Entity:** `Ward Bed Allocations`
...
1. **Complete RESTful CRUD API:**
   - `GET /api/inpatients` — List & search records with multi-field queries.
   - `GET /api/inpatients/:id` — Retrieve detailed entity record.
   - `POST /api/inpatients` — Create new validated entity with auto-timestamps.
   - `PUT /api/inpatients/:id` — Modify existing record with verification.
   - `DELETE /api/inpatients/:id` — Remove entity from persistent storage.

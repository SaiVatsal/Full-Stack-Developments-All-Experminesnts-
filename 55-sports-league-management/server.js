/**
 * ArenaPro Sports League Management - Backend REST API Server
    ...
    if (!fs.existsSync(VERCEL_TMP_DB)) {
      try {
        if (fs.existsSync(DB_FILE)) {
          ...
          fs.writeFileSync(VERCEL_TMP_DB, JSON.stringify({ fixtures: [], activityRecords: [] }), 'utf8');
        }
      } catch (e) {}
    }
...
function readDb() {
  try {
    ...
    return { fixtures: [], activityRecords: [] };
  }
}
...
app.get('/api/health', (req, res) => {
  res.json({
    status: 'online',
    project: 'ArenaPro Sports League Management',
...
app.get('/api/fixtures/:id', (req, res) => {
  const db = readDb();
  const item = (db.fixtures || []).find(i => String(i.id) === String(req.params.id));
  if (!item) {
    return res.status(404).json({ error: 'League Match Fixture not found' });
  }
  res.json(item);
});
...
app.delete('/api/fixtures/:id', (req, res) => {
  ...
  db.fixtures = filtered;
  writeDb(db);

  res.json({ message: 'League Match Fixture deleted successfully', id: req.params.id });
});
...
if (require.main === module) {
  app.listen(PORT, () => {
    console.log('======================================================');
    console.log('🚀 ArenaPro Sports League Management is running!');
    console.log('📂 Category: Sports Management');
